...
function deplacerPinces(direction) {
  ...
  if (window.innerWidth < 480) {
    ...
    if (direction === "next" && positionActuelle < pinces.length - 2) {
      positionActuelle += 2;
      linge.style.transform = `translateX(-${
        positionActuelle * (largeurConteneur / 2)
      }px)`;
    } else if (direction === "prev" && positionActuelle > 0) {
      positionActuelle -= 2;
      linge.style.transform = `translateX(-${
...
const clickBainTexte = document.querySelector("#click-bain");
let msgBain = document.querySelector(".message");
let pince1 = document.querySelector("#pince_1");
let firstClick = true;

function handleBainClick() {
  if (firstClick) {
    clickBainTexte.classList.add("invisible"); // Cache le texte "Click"
    msgBain.classList.remove("invisible");
    if (window.innerWidth > 480) {
      pince1.classList.add("animated");
    }
    firstClick = false; // Désactive cette partie pour les prochains clics
  }
...
photosLogiques.forEach((photo, index) => {
  photo.addEventListener("click", function () {
    ...
    msgBain.classList.add("invisible");
    ...
    setTimeout(() => {
      try {
        ...
        const marginLeft =
          pinceCoords.left +
          pinceCoords.width / 2 -
          (photoCoords.left + photoCoords.width / 2);
        const marginTop =
          pinceCoords.top + pinceCoords.height / 2 - photoCoords.top;
        ...
        const updatePhotoPosition = () => {
          ...
          photo.style.top = `${
            pinceCoords.top + pinceCoords.height / 2 - photoCoords.height / 2
          }px`;
        };
    ...
    onmove: function (event) {
      ...
      if (pince) {
        ...
        const distance = Math.sqrt(
          Math.pow(photoX - pinceX, 2) + Math.pow(photoY - pinceY, 2)
        );
